...
--*/
{
    ...
    gData.DriverObject = DriverObject;
    gData.Filter = NULL;
    gData.ServerPort = NULL;
    gData.ClientPort = NULL;
    try {
        //
        ...
        status = FltRegisterFilter(gData.DriverObject,
            &FilterRegistration,
            &gData.Filter);
        ...
        status = MnFltPrepareServerPort(sd);
        ...
        status = FltStartFiltering(gData.Filter);
        ...
        if (!NT_SUCCESS(status)) {
            ...
            if (NULL != gData.ServerPort) {
                FltCloseCommunicationPort(gData.ServerPort);
            }

            if (NULL != gData.Filter) {
                FltUnregisterFilter(gData.Filter);
            }
...
--*/
{
    ...
    case FLT_STREAM_CONTEXT:
        ...
        status = FltAllocateContext(gData.Filter,
            FLT_STREAM_CONTEXT,
            sizeof(DF_STREAM_CONTEXT),
...
--*/
{
    ...
    status = FltCreateFileEx2(gData.Filter,
        Data->Iopb->TargetInstance,
        &handle,
        NULL,
...
)
{
    ...
    TFR_DATA* transferData = NULL;
    LARGE_INTEGER timeout = { 0 };
    if (Data->Iopb->TargetFileObject->FileName.Length >= MAX_FILE_PATH)
    {
        return STATUS_UNSUCCESSFUL;
    }

    transferData = ExAllocatePoolZero(DF_CONTEXT_POOL_TYPE,
        sizeof(TFR_DATA),
        DF_STRING_POOL_TAG);
